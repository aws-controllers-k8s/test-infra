...
func (s *Server) processAgentCommand(commandText string, repo *githubclient.Repository, issue *githubclient.Issue, postComment func(string) error) error {
	...
	// Create pseudo-PullRequest for ProwJob generation
	pr := s.createPseudoPR(repo, issue)
	...
	prowJob, err := s.prowJobGenerator.CreateWorkflowProwJob(agentCmd.WorkflowName, agentCmd.Args, agentCmd.Flags, pr, timeout)
	if err != nil {
...
// createPseudoPR creates a pseudo-PullRequest from issue for ProwJob generation
func (s *Server) createPseudoPR(repo *githubclient.Repository, issue *githubclient.Issue) *githubclient.PullRequest {
	defaultBranch := s.getDefaultBranch(repo)
	latestSHA := s.getLatestCommitSHA(repo, defaultBranch)
	return &githubclient.PullRequest{
		Number: issue.Number,
		User:   issue.User,
		Base: &githubclient.PullRequestBranch{
			Ref:  &defaultBranch,
			SHA:  &latestSHA,
			Repo: repo,
		},
		Head: &githubclient.PullRequestBranch{
			SHA: &latestSHA,
		},
	}
}